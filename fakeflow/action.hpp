...
inline std::ostream&
operator<<(std::ostream& os, Action const& a)
{
  Action::Action_data const& data = a.value;
  Byte* buf = nullptr;
  uint32_t val = 10;
  ...
  switch (a.type)
  {
    case Action::Type::SET:
      buf = new Byte[data.set.field.length];
      std::fill(std::copy(data.set.value, data.set.value + data.set.field.length, buf),
                buf + sizeof(uint32_t), 0);
      val = *reinterpret_cast<uint32_t*>(buf);

      os << "SET: " << data.set.field.offset << ","
    ...
    case Action::Type::OUTPUT:
      os << "OUTPUT PORT: " << data.output.port << '\n';
      break;
    case Action::Type::QUEUE:
      os << "QUEUE\n";
      break;
