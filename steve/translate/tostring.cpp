...
std::string const
tostring(Unary_kind op)
{
  switch (op) {
    // Object operators
    case deref_op: return "*";   // *e
    case addr_op: return "&";    // &e
    // Arithmetic operators
    case pos_op: return "+";     // +e
    case neg_op: return "-";     // -e
    // Bitwise operators
    case bnot_op: return "~";    // ~e
    // Assignment operators
    case preinc_op: return "++";  // ++e
    case predec_op: return "--";  // --e
    case postinc_op: return "++"; // e++
    case postdec_op: return "--"; // e--
    // Logical operators
    case not_op: return "!";   // !e
  }
...
// Foreign declaration
std::string const
tostring(Foreign_cpp_decl const* d)
{
  return "extern \"C++\" " + tostring(d->decl());
}
...
tostring(Decl const* d)
{
  switch (d->kind) {
    ...
    case foreign_cpp_decl: return tostring(as<Foreign_cpp_decl>(d));
    default:
      return "<error>";
  }
...
tostring(Type const* t)
{
  switch (t->kind) {
    ...
    case pointer_type: return tostring(as<Pointer_type>(t));
    default:
      return "<error type>";
  }
}
...
// op e1
std::string const
tostring(Unary_expr const* e)
{
  return tostring(e->op()) + tostring(e->arg());
}
...
std::string const
tostring(Dynamic_cast_expr const* e)
{
  return "dynamic_cast<" + tostring(e->cast_type()) + ">(" + tostring(e->object()) + ")";
}


std::string const
tostring(Reinterpret_cast_expr const* e)
{
  return "reinterpret_cast<" + tostring(e->cast_type()) + ">(" + tostring(e->object()) + ")";
}
...
tostring(Expr const* e)
{
  ...
  switch (e->kind) {
    ...
    case unary_expr: return tostring(as<Unary_expr>(e));
    ...
    case dynamic_cast_expr: return tostring(as<Dynamic_cast_expr>(e));
    case reinterpret_cast_expr: return tostring(as<Reinterpret_cast_expr>(e));
    default:
      return "<error expr>";
  }
